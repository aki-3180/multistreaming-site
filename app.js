...
const CHAT_BASE_W = 290;
// 埋め込みチャットには内部の最小幅があり、これを下回る論理幅で描画すると
// 右端（YouTubeなら絵文字・送信ボタンやメニュー）がはみ出して見切れる。
// 縮小率の下限で論理幅が痩せないよう、プラットフォームごとに必要幅を持つ。
//
// 拡大率は「パネル実寸 ÷ 論理幅」で、本文13pxがその倍率で描画される。
// スマホ横画面のパネル実寸188pxだと 290 で約8.4px、353 で約6.9px。
// Twitch・Kickは論理幅268pxでも見切れなかった実績があるので詰められるが、
// YouTubeは268pxで見切れたので、そこから十分離した値に留めている。
const CHAT_LOGICAL_W = { yt: 353, tw: 290, kick: 290 };
